...
use crate::{
    auth::{
        ...
        save_token_to_secrets, SECRET_TWITCH_ACCESS_TOKEN, SECRET_TWITCH_CLIENT_ID,
        SECRET_TWITCH_CLIENT_SECRET, SECRET_TWITCH_CSRF_TOKEN, SECRET_TWITCH_REFRESH_TOKEN,
        SECRET_TWITCH_SCOPES,
    },
...
pub async fn validate_channel(
    ...
    let is_moderator = if is_owner {
        ...
        let request =
            get_moderated_channels::GetModeratedChannelsRequest::user_id(token_user_id.as_str())
                .first(100);
        let response = auth
            .helix_client()
            .req_get(request, &token)
    ...
    if !is_moderator {
        return Ok(ValidateChannelResult {
            ...
            channel_id: Some(channel_id_str),
            display_name: Some(channel_display.clone()),
            message: "You must be the channel owner or a moderator to add this channel".to_string(),
        });
    }
    ...
    if !missing.is_empty() {
        return Ok(ValidateChannelResult {
            ...
            display_name: Some(channel_display.clone()),
            message: format!(
                "Missing scopes: {}. Please re-authorize with the required scopes.",
                missing.join(", ")
            ),
        });
    }
...
/// Minimum scopes required to add a channel (owner or moderator); same as used by validate_channel.
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RequiredChannelScopes {
    /// Scopes required to add any channel you own or moderate.
    pub required: Vec<String>,
    /// Additional scope needed to add channels you moderate (not own).
    pub recommended_for_moderated: Vec<String>,
}

/// Returns the minimum required Twitch scopes for adding channels (used by Channels UI).
#[tauri::command]
pub async fn get_required_channel_scopes() -> RequiredChannelScopes {
    let required = core_scopes().into_iter().map(|s| s.to_string()).collect();
    let recommended_for_moderated =
        vec![twitch_api::twitch_oauth2::Scope::UserReadModeratedChannels.to_string()];
    RequiredChannelScopes {
        required,
        recommended_for_moderated,
    }
}
